...
from meme_cam.ui import draw_debug, draw_landmarks, meme_panel, side_by_side
...
def main() -> None:
    ...
    ap.add_argument("--window", type=int, default=config.SMOOTH_WINDOW,
                    help="frames to average")
    ap.add_argument("--hold", type=int, default=config.HOLD_FRAMES,
                    help="frames a meme must win before it is shown")
    ...
    stab = MatchStabilizer(labels, threshold=args.threshold,
                           window=args.window, hold_frames=args.hold)
    ...
    try:
        while True:
            ...
            meme = memes.get(shown) if shown else None
            caption = None
            if shown:
                p = float(stab.smoothed[labels.index(shown)])
                caption = f"{shown} ({p:.0%})"
            panel = meme_panel(meme, layout.panel_w, layout.height, caption=caption)
